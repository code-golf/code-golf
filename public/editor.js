...
// node_modules/nim-codemirror-mode/index.js
var require_nim_codemirror_mode = __commonJS({
  "node_modules/nim-codemirror-mode/index.js"(exports, module) {
    module.exports.nim = function nim3(conf, parserConf) {
      let ERRORCLASS = "error";
      function wordRegexp2(words4) {
        return new RegExp("^((" + words4.join(")|(") + "))\\b");
      }
      let operators6 = new RegExp("\\=\\+\\-\\*\\/\\<\\>\\@\\$\\~\\&\\%\\|\\!\\?\\^\\:\\\\");
      let identifiers3 = new RegExp("^[_A-Za-z][_A-Za-z0-9]*");
      let commonkeywords = [
        "addr",
        "asm",
        "bind",
      ...
      let keywords11 = wordRegexp2(commonkeywords);
      let builtins6 = wordRegexp2(commonBuiltins);
      let indentInfo = null;
      let stringPrefixes2 = new RegExp(`^(('{3}|"{3}|['"]))`, "i");
      function tokenBase9(stream, state) {
        ...
        if (stream.match(operators6))
          return "operator";
        if (stream.match(keywords11))
          return "keyword";
        if (stream.match(builtins6))
          return "builtin";
        if (stream.match(identifiers3)) {
          if (state.lastToken != null && state.lastToken.match(/proc|iterator|macro|template|class|converter|func/)) {
            return "def";
          }
          return "variable";
        }
      ...
      function tokenStringFactory2(delimiter) {
        ...
        tokenString5.isString = true;
        return tokenString5;
      }
      function indent2(stream, state, type2) {
        type2 = type2 || "nim";
        let indentUnit2 = 0;
        if (type2 === "nim") {
          ...
          for (let i = 0; i < state.scopes.length; ++i) {
            if (state.scopes[i].type === "nim") {
              indentUnit2 = state.scopes[i].offset + conf.indentUnit;
              break;
            }
          }
        } else {
          indentUnit2 = stream.column() + stream.current().length;
        }
      ...
      function dedent2(stream, state, type2) {
        type2 = type2 || "nim";
      ...
      function tokenLexer(stream, state) {
        ...
        } else if (stream.eol() && current.match(/\=|\:|import|include|type|const|let|let/)) {
        ...
        if (delimiter_index !== -1) {
          if (dedent2(stream, state, current)) {
            return ERRORCLASS;
          }
        }
    ...
    module.exports.register = (CodeMirror2) => {
      CodeMirror2.defineMode("nim", nim);
      CodeMirror2.defineMIME("text/x-nim", "nim");
      CodeMirror2.modeInfo.push({
        name: "Nim",
        mime: "text/x-nim",
        mode: "name",
        ext: ["nim"]
    ...
    var LZString2 = function() {
      ...
      var LZString3 = {
        ...
        _decompress: function(length, resetValue, getNextValue) {
          var dictionary = [], next2, enlargeIn = 4, dictSize = 4, numBits = 3, entry = "", result = [], i, w, bits, resb, maxpower, power, c2, data = { val: getNextValue(0), position: resetValue, index: 1 };
          for (i = 0; i < 3; i += 1) {
            dictionary[i] = i;
          }
          bits = 0;
...
var Transaction = class {
  constructor(startState, changes, selection2, effects, annotations, scrollIntoView) {
...
var EditorState = class {
  constructor(config2, doc2, selection2, tr = null) {
    this.config = config2;
    this.doc = doc2;
    this.selection = selection2;
    this.applying = null;
  ...
  static create(config2 = {}) {
    ...
    return new EditorState(configuration, doc2, selection2);
  }
  get tabSize() {
...
function skipAtoms(view, oldPos, pos) {
  let atoms4 = view.pluginField(PluginField.atomicRanges);
  for (; ; ) {
    let moved = false;
    for (let set of atoms4) {
...
var MouseSelection = class {
  ...
  select(event) {
    ...
    if (!selection2.eq(this.view.state.selection) || selection2.main.assoc != this.view.state.selection.main.assoc)
      this.view.dispatch({
  ...
  if (cursor2 && (dir < 0 ? cursor2.childBefore(token2.from) : cursor2.childAfter(token2.to)))
    do {
      if (dir < 0 ? cursor2.to <= token2.from : cursor2.from >= token2.to) {
        if (depth2 == 0 && matching2.indexOf(cursor2.type.name) > -1) {
          ...
          depth2--;
          if (depth2 == 0)
            return { start: firstToken, end: { from: cursor2.from, to: cursor2.to }, matched: false };
        }
      }
...
function setSel(state, selection2) {
  return state.update({ selection: selection2, scrollIntoView: true, annotations: Transaction.userEvent.of("keyboardselection") });
}
...
function highlightTreeRange(tree, from, to, style, span) {
  ...
  let cursor2 = tree.topNode.cursor;
  ...
  function node(inheritedClass, depth2, scope) {
    ...
    let upto = start;
    if (!opaque && cursor2.firstChild()) {
      do {
        if (cursor2.from > upto && spanClass != cls)
          flush(upto, cls);
        upto = cursor2.to;
...
var HistoryState = class {
  ...
  addSelection(selection2, time, userEvent, newGroupDelay) {
    ...
    if (last.length > 0 && time - this.prevTime < newGroupDelay && userEvent == "keyboardselection" && this.prevUserEvent == userEvent && eqSelectionShape(last[last.length - 1], selection2))
      return this;
...
var unescapeString = (string3) => string3.slice(1, -1).replace(/\\x[0-9a-f]{1,2}/ig, (x) => String.fromCharCode(parseInt(x.slice(2), 16))).replace(/\\[0-7]{1,3}/g, (x) => String.fromCharCode(parseInt(x.slice(1), 8) & 255)).replace(/\\u[0-9a-f]{1,8}/ig, (x) => {
  try {
  ...
  try {
    ...
    if (token[0] === "'" && token[token.length - 1] === "'") {
      ...
      if (token.length > 1 && !isNaN(token.slice(0, -1))) {
        ...
        let symbol3 = { name: token, pos: codePos };
        next();
        return { value: symbol3, floatPrec };
      }
...
function cutAt(tree, pos, side) {
  let cursor2 = tree.cursor(pos);
  for (; ; ) {
    if (!(side < 0 ? cursor2.childBefore(pos) : cursor2.childAfter(pos)))
      for (; ; ) {
        if ((side < 0 ? cursor2.to < pos : cursor2.from > pos) && !cursor2.type.isError)
          return side < 0 ? Math.max(0, Math.min(cursor2.to - 1, pos - 5)) : Math.min(tree.length, Math.max(cursor2.from + 1, pos + 5));
...
// editor.js
var import_nim_codemirror_mode = __toModule(require_nim_codemirror_mode());
...
var extensions = {
  ...
  "nim": StreamLanguage.define((0, import_nim_codemirror_mode.nim)({}, {})),
...
var export_LZString = import_lz_string.default;
export {
  EditorState,
  EditorView,

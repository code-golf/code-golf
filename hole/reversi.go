...
//go:embed reversi-tests.txt
var casesTxt string

func getStaticTestCases() []ReversiBoard {
	tokens := strings.Split(strings.Trim(casesTxt, "\n"), "\n\n")

	tests := make([]ReversiBoard, len(tokens)*4)
	for test := range tokens {
		lines := strings.Split(tokens[test], "\n")
		...
		tests[test*4] = board
		tests[test*4+1] = flipReversiBoard(board, true, false)
		tests[test*4+2] = flipReversiBoard(board, false, true)
		tests[test*4+3] = flipReversiBoard(board, true, true)
	}

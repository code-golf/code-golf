...
type NFAStateTransition struct{ input, stateName string }
...
func solveNFA(test string) string {
  ...
  for _,line := range lines[firstTestRow:]{
    ...
    if line != "ε" {
      inputString := line[1:len(line)-1]
      nfaResult = recurseNFA(startState, inputString, stateMap)
    }
    ...
    if len(nfaResult) == 0 {
      ...
      for stateName,_ := range resultStateNamesMap{
...
//solve recursively
func recurseNFA(currentState NFAState, input string, stateMap map[NFAStateTransition][]NFAState) []NFAState {
  //fmt.Println("recursive  ",currentState, input)
  if len(input) == 1 {
    //fmt.Println("terminal: ", stateMap[NFAStateTransition{input, currentState.name}])
  ...
  for _,nextState := range stateMap[NFAStateTransition{string(input[0]), currentState.name}] {
    result := recurseNFA(nextState, input[1:], stateMap)
...
func generateNFA() string {
  ...
  for j,state := range states[:stateLength]{shuffleStates[j] = state}
  ...
  for i := 0; i < stateLength; i++ {
    ...
    for j := 0; j < alphabetLength; j++ {
      ...
      if rand.Intn(4) == 0 {
        ...
        for j,state := range shuffleStates[:possibleStatesCount]{chosenStates[j] = state}

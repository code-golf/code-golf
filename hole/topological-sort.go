...
func tsort(in string) string {
	...
	for pair := range slices.Chunk(strings.Fields(in), 2) {
		...
		in_degrees[pair[0]] = in_degrees[pair[0]]
		in_degrees[pair[1]]++
	}
	var stack []string
...
func generateTsortTest(length, additional int) string {
	...
	for i, x := range init[:length-1] {
		seq = append(seq, x, init[i+1])
		rows++
	}
	arc_choices := rand.Perm((length - 2) * (length - 1) / 2)
	for i, x := range init[:length-2] {
		for _, y := range init[i+2:] {
			if arc_choices[len(arc_choices)-1] < additional {
				seq = append(seq, x, y)
				rows++
			}

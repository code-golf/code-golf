...
is-deeply settings, {
    :country(Str), :layout<default>, :referrer_id(Int), :!show_country,
    :theme<auto>,  :time_zone(Str),
}, 'DB has defaults';

for <country layout time_zone> {
    throws-like { post %( :country(''), :layout<default>, :time_zone(''), $_ => 'baz' ) },
        Exception, message => /'400 Bad Request'/, "400 with invalid $_";
}

post my %args = :country<GB>, :layout<tabs>, :show_country<on>, :theme<dark>,
...
sub settings { $dbh.execute(q:to/SQL/).row :hash }
    SELECT country, layout, referrer_id, show_country, theme, time_zone
      FROM users
     WHERE id = 123
SQL

...
async function fetchSolutions() {
    ...
    const resultsHoles = [...new Set(results.map(x => x.hole))].map(x => holes[x]);
    const resultsLangs = [...new Set(results.map(x => x.lang))].map(x => langs[x]);
    $('#resultsOverview').innerText = results.length === 0
        ? '0 matches'
        : `${amount(totalCount, 'match', 'matches')} across ${amount(results.length, 'solution')} (${amount(resultsHoles.length, 'hole')} in ${resultsLangs.length > 5 ? `${resultsLangs.length} languages` : resultsLangs.join(', ')})`;

...
function wasm(url: string): Promise<WebAssembly.Module> {
...
function assemblyIdeFactory(as: string, ld: string, objdump: string, asArgs: (src: string) => string[], ldArgs: (src: string) => string[]) {
    ...
    return async () => assemblyIde(new ExtAsmState(await PreprocessedAssembler.create(wasm(aspp),
        ToolchainAssembler.create(wasm(as), wasm(ld), wasm(objdump)).then(x =>
